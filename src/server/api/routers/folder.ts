...
import { generateFolderPreviews, scanFolder } from "~/utils/scanFolder";
...
export const folderRouter = createTRPCRouter({
  ...
  scanFolder: publicProcedure
    ...
    generateFolderPreviews: publicProcedure
    .input(z.object({ id: z.number() }))
    .mutation(async ({ ctx, input }) => {
        const folder = await ctx.db.folder.findUnique({
          where: {
        ...
        if (folder?.folderPath) {
          await generateFolderPreviews(folder, folder.folderPath, ctx.db)
          //update last scanned at?
          return true
        } else {
          return false
